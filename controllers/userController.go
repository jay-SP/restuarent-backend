...
import (
	...
	helpers "github.com/jay-SP/restuarant-management-backend/helpers"
...
func Signup() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		//you'll check if the email ha already been used by another user
		...
		//check if the phone no. already been used by another user
...
func Login() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		//convert the login data from postman which is in JSON to golang readble format
		if err := c.BindJSON(&user); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
		}
		//find a user with the email and see if that user even exists
		...
		//all if all goes well, then you will generate tokens
