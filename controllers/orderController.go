...
func UpdateOrder() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		orderID := c.Param("order_id") //picks up orderid
		...
		order.Updated_at, _ = time.Parse(time.RFC3339, time.Now().Format(time.RFC3339)) //RFC3339 format"2006-01-02T15:04:05Z07:00"
...
// validation needs to there whenever u creating something
func CreateOrder() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if order.Table_id != nil {
			...
			defer cancel()
			if err != nil {
				msg := fmt.Sprintf("messsage:Table was not found")
				c.JSON(http.StatusInternalServerError, gin.H{"error": msg})
				return
			}
		}
