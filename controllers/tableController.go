...
func UpdateTable() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		tableId := c.Param("table_id") // recieve from frontend
		...
		if table.Table_number != nil {
			updateObj = append(updateObj, bson.E{Key: "table_numer", Value: table.Table_number})
		}
		...
		result, err := tableCollection.UpdateOne(
			...
			filter,
			bson.D{
				{"$set", updateObj}, // Add a comma after "$set"
			},
			&opt,
		)
...
func CreateTable() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		//we validate before we send anything to db
		...
		if insertErr != nil {
			msg := fmt.Sprintf("Table item was not creatd")
			c.JSON(http.StatusInternalServerError, gin.H{"error": msg})
			return
		}
		defer cancel()
